...
check(
    "place_sl_tp_orders idempotent (2nd call no-op)",
    r2.get("skipped") == "already_placed",
)
...
trade = {
    "trade_id": tB,
    "legs": [dict(l) for l in legsB],
    "sl": {"pe": 120.0, "ce": 117.0},
    "tp": {"pe": 40.0, "ce": 39.0},
    "cumulative_pnl": 0,
}
...
action_pe_sl = {
    ...
    "ltp": 130.0,
    "reason": "SL: PE24500 LTP=130 >= SL=120",
}
...
check(
    "CE side intact (2 legs) after PUT SL",
    sum(1 for l in trade["legs"] if l["type"] == "CE") == 2,
)

ledger = json.loads(oa.LEDGER_FILE.read_text())
exits = {
    o["symbol"]: o["action_type"]
    for o in ledger["orders"].values()
    if o["order_type"] == "EXIT"
}
check("EXIT order placed for short PE (BUY-to-close)", exits.get("B-PE-SELL") == "BUY")
check(
    "EXIT order placed for HEDGE PE (SELL-to-close)", exits.get("B-PE-HEDGE") == "SELL"
)

tdb.update_active_trade(tB, legs=trade["legs"], sl=trade["sl"], tp=trade["tp"])
check(
    "trade still ACTIVE after one-side close (CE open)", tdb.has_active_trades() is True
)
...
action_ce_tp = {
    ...
    "ltp": 38.0,
    "reason": "TP: CE24500 LTP=38 <= TP=39",
}
...
with tdb._connect() as con:
    hist = con.execute(
        "SELECT close_reason FROM trade_history WHERE trade_id=?", [tB]
    ).fetchall()
...
tradeC = {
    "trade_id": tC,
    "legs": attach_ltp(legsC, 150.0, 10.0),
    "sl": {"pe": 120.0},
    "tp": {"pe": 40.0},
    "cumulative_pnl": 0,
}
act_floor = {
    "type": pm.CLOSE_ALL,
    "priority": 6,
    "reason": "Cumulative P&L below floor",
}
...
with tdb._connect() as con:
    histc = con.execute(
        "SELECT close_reason FROM trade_history WHERE trade_id=?", [tC]
    ).fetchall()
